...
//Say goodbye to completely blank columns (except the first and last)
...
export async function FinishCrossword(layoutString, rowSize) {
  ...
  document.querySelectorAll('input[type="text"]').forEach((input) => {
    ...
    if (input.value != "") inputString = inputString.concat(input.value[0]);
    else inputString = inputString.concat("-");
  ...
  for (let i = 0; i < rowSize; i++) {
    for (let j = 0; j < rowSize; j++) {
      ...
      if (layoutString[layoutIndex] != "-") {
        ...
        } else if (inputString[inputIndex] && layoutString[layoutIndex]) {
  ...
  if (allCorrect) {
    ...
    if (
      confirm(
        "Nice! You got all the words within " +
          time.toString() +
          " seconds! Do another?"
      )
    ) {
